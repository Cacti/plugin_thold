...
function thold_view_show() {
	...
	load_current_session_value("i_page", "sess_thold_view_host_current_page", "1");
	load_current_session_value("page", "sess_thold_view_host_current_page", "1");
	load_current_session_value("sort_column", "sess_thold_view_host_sort_column", "thold_alert");
	load_current_session_value("sort_direction", "sess_thold_view_host_sort_direction", "DESC");
	...
	$result = db_fetch_assoc("SELECT plugin_thold_log.*, host.description as hostdesc, thold_data.rra_id, thold_data.data_id
					 ...
					 WHERE thold_data.host_id=host.id AND thold_data.id = plugin_thold_log.threshold_id
					 AND plugin_thold_log.type = 1 $sort $limit");
	$total_rows = db_fetch_cell("SELECT count(*)
					 FROM plugin_thold_log
					 WHERE AND plugin_thold_log.type = 1");
	...
	if (sizeof($result) > 0) {
		foreach ($result as $row) {
			form_alternate_row_color($colors["alternate"],$colors["light"],$i); $i++;
				?>
				<td>
					<?php print "<p class='linkEditMain'><a href='thold.php?rra=" . $row['rra_id'] . '&view_rrd=' . $row['data_id'] . "'>" . $row["threshold_id"] . "</a></p>";?>
				</td>
