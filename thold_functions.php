...
function thold_substitute_snmp_query_data($string, $device_id, $snmp_query_id, $snmp_index, $max_chars = 0) {
	...
	$snmp_cache_data = db_fetch_cell_prepared('SELECT field_value
		...
		AND snmp_index = ?
		AND field_name= ?',
		[$device_id, $snmp_query_id, $snmp_index, $field_name]);
 ...
 * @param mixed $l_escape
 * @param mixed $r_escape
 *
	...
	switch ($thold_data['thold_type']) {
		case 0:	// hi/low
			...
			if ($breach_up || $breach_down) {
					...
					if (!$suspend_notify && !$maint_dev) {
						...
						if ($thold_snmp_traps) {
							...
							$thold_snmp_data['eventDescription']         = str_replace(
								['<FAIL_COUNT>', '<FAIL_DURATION>'],
								[$thold_snmp_data['eventFailCount'], $thold_snmp_data['eventFailDuration']],
				...
				WHERE id = ?',
					[$thold_data['thold_alert'], $thold_data['thold_fail_count'], $thold_data['id']]);
				...
				thold_fail_count = 0
				WHERE id = ?',
					[$thold_data['thold_alert'], $thold_data['thold_warning_fail_count'], $thold_data['id']]);
			} else {
				...
				case 0:		// all clear
					...
					if ($bl_alert_prev != 0) {
						...
						if ($thold_data['bl_fail_count'] >= $bl_fail_trigger && $thold_data['restored_alert'] != 'on' && !$maint_dev) {
							...
							db_execute_prepared('UPDATE thold_data SET lastchanged = NOW() WHERE id = ?', [$thold_data['id']]);
		...
		case 2:	// time based
			...
			if ($breach_up || $breach_down) {
				...
				ORDER BY time DESC
				LIMIT 1',
					[$thold_data['id'], ST_NOTIFYRA, ST_NOTIFYAL]);
				...
				thold_fail_count = ?
				WHERE id = ?',
					[$thold_data['thold_alert'], $failures, $thold_data['id']]);
			} elseif ($warning_breach_up || $warning_breach_down) {
				$notify = false;
				...
				thold_fail_count = ?
				WHERE id = ?',
					[$thold_data['thold_alert'], $warning_failures, $failures, $thold_data['id']]);
			} else {
				...
				if ($alertstat != 0 && $warning_failures < $warning_trigger && $thold_data['restored_alert'] != 'on') {
					...
					thold_warning_fail_count = ?
					WHERE id = ?',
						[$failures, $warning_failures, $thold_data['id']]);
				}
			}
	...
	while ($cursor < $x) {
		...
		switch($type) {
			...
			case 2:
				...
				array_push($stack, ['type' => 6, 'value' => $result]);
...
function delete_old_thresholds() {
	...
	if (cacti_sizeof($tholds)) {
		foreach ($tholds as $thold_data) {
			plugin_thold_log_changes($thold_data['id'], 'deleted', ['message' => 'Auto-delete due to Data Source removal']);
 ...
 * @param mixed $ref_time
 * @param mixed $ref_range
 */
...
function thold_check_baseline($local_data_id, $data_source_name, $current_value, &$thold_data) {
	...
	db_execute_prepared('UPDATE thold_data
		...
		WHERE id = ?',
		[$ref_value_min, $ref_value_max, $ref_value_avg, $ref_value_last, $thold_data['id']]);
...
function get_bl_type($type, $cf) {
	...
	switch($type) {
		...
		case '4':
		case '6':
			return str_replace(['Deviation', 'Absolute'], ['Dev', 'Abs'], $bl_types[$type]);
...
function save_thold() {
		...
		if (!isset_request_var('local_graph_id')) {
			...
			if ($graph_array !== false) {
				...
				$temp = db_fetch_cell_prepared('SELECT dtr.id
					...
					[get_request_var('local_data_id'), $template['data_source_name']]);
	...
	if ($template_enabled == 'on') {
		...
		if (get_request_var('id') > 0) {
			db_execute_prepared('UPDATE thold_data
				...
				WHERE id = ?',
				[$thold_per_enabled, get_request_var('id')]);
		}
		...
		plugin_thold_log_changes($data['id'], 'modified', ['id' => $data['id'], 'template_enabled' => 'on']);
	...
	if (isset_request_var('snmp_event_category')) {
		set_request_var('snmp_event_category', trim(str_replace(['\\', "'", '"'], '', get_nfilter_request_var('snmp_event_category'))));
	}

	if (isset_request_var('snmp_event_description')) {
		set_request_var('snmp_event_description', trim(str_replace(['\\', "'", '"'], '', get_nfilter_request_var('snmp_event_description'))));
	}
	...
	if (isset_request_var('notify_accounts') && is_array(get_nfilter_request_var('notify_accounts'))) {
		...
		thold_save_threshold_contacts($id, []);
	}
	...
	if ($id) {
		...
		$thold = db_fetch_row_prepared($thold_sql, [$id]);
					...
					foreach ($data_sources as $data_source) {
						...
						if ($data_source['snmp_query_id'] > 0) {
							$exists = db_fetch_cell_prepared('SELECT id
								...
								AND thold_template_id = ?
								AND data_template_rrd_id = ?',
								[$local_data_id, $template['id'], $data_template_rrd_id]);
						} else {
							$exists = false;
						}
...
/**
 * Verify that a live PID was already running when its process row was created.
 *
 * @param int           $pid
 ...
 * @param callable|null $identity Optional process-start probe used by tests.
 *
 * @return bool
 */
function thold_notification_register_process($thread, $timeout = 3600, $lock = null, $probe = null, $identity = null) {
	global $config;
...
function thold_notification_complete($sql, array $params, array $ids, $pid) {
	...
	if (!$updated) {
		$message = sprintf(
			'ERROR: Notification process %s could not complete queue row(s) %s because the database update failed.',
			(int) $pid,
			implode(', ', $ids)
		);
...
/**
 * Bound queue error text to the notification_queue.error_message column.
 *
 * @param mixed $message
 *
 * @return string
...
function thold_notification_reject_unknown_topic($id, $pid, $topic) {
	$message = thold_notification_error_message('Unsupported notification topic: ' . (string) $topic);
			...
			switch($topic) {
				...
				case 'thold_uhost_mail':
					...
					if (cacti_sizeof($attachments)) {
						foreach ($attachments as $index => $attach) {
							$attachments[$index]['attachment'] = base64_decode($attach['attachment'], true);
						}
					}
					...
					if (!$one_email) {
						...
						thold_notification_complete('UPDATE notification_queue
							...
							WHERE id = ?
							AND process_id = ?',
							[$error_code, thold_notification_error_message($error), $nend - $nstart, $r['id'], $pid],
							[$r['id']],
							$pid);
					} else {
						...
						if (!isset($emails[$id])) {
							...
							$emails[$id]['body']          = str_replace(['<body>', '</body>'], '', $body);
				...
				case 'thold_uhost_cmd':
					...
					thold_notification_complete('UPDATE notification_queue
						...
						WHERE id = ?
						AND process_id = ?',
						[$return, thold_notification_error_message(implode("\n", $output)), $nend - $nstart, $r['id'], $pid],
						[$r['id']],
						$pid);
		...
		if (cacti_sizeof($emails) && $one_email) {
			foreach ($emails as $email) {
				...
				thold_notification_complete("UPDATE notification_queue
					...
					WHERE id IN ($ids)
					AND process_id = ?",
					[$error_code, thold_notification_error_message($error), $nend - $nstart, $pid],
					$email['ids'],
					$pid);
			}
		}
...
function thold_threshold_ack($id) {
	if (api_user_realm_auth('thold.php')) {
		if (is_thold_allowed($id)) {
			plugin_thold_log_changes($id, 'acknowledge_threshold', ['id' => $id]);
...
function thold_threshold_suspend_ack($id) {
	if (api_user_realm_auth('thold.php')) {
		if (is_thold_allowed($id)) {
			plugin_thold_log_changes($id, 'suspend_acknowledge_threshold', ['id' => $id]);
...
function thold_threshold_resume_ack($id) {
	if (api_user_realm_auth('thold.php')) {
		if (is_thold_allowed($id)) {
			plugin_thold_log_changes($id, 'resume_acknowledge_threshold', ['id' => $id]);
...
function thold_get_allowed_devices($sql_where = '', $order_by = 'description', $sql_limit = '', &$total_rows = 0, $user_id = 0, $device_id = 0) {
	...
	if (function_exists('get_total_row_data') && $device_id == 0) {
		$total_rows = get_total_row_data($user_id, $sql, [], 'thold_device', 10);
	} else {
		$total_rows = db_fetch_cell($sql);
	}
