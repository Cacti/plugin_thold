...
function form_thold_filter() {
	...
	if (cacti_sizeof($data_templates)) {
		foreach ($data_templates as $data_template) {
			print "<option value='" . $data_template['id'] . "'";

			if (get_request_var('data_template_id') == $data_template['id']) {
				print ' selected';
			} print '>' . html_escape($data_template['name']) . '</option>';
		}
	}
	?>
						</select>
...
function thold_show_log() {
	...
	if (cacti_sizeof($logs)) {
		foreach ($logs as $l) {
			...
			$data_type = db_fetch_cell_prepared('SELECT data_type FROM thold_data WHERE id = ?', [$l['threshold_id']]);
			...
			$show_units   = (db_fetch_cell_prepared('SELECT show_units FROM thold_data WHERE id = ?', [$l['threshold_id']]) ? true : false);
			$units_suffix = db_fetch_cell_prepared('SELECT units_suffix FROM thold_data WHERE id = ?', [$l['threshold_id']]);
			$decimals     = db_fetch_cell_prepared('SELECT decimals FROM thold_data WHERE id = ?', [$l['threshold_id']]);
...
function form_thold_log_filter() {
	...
	if (cacti_sizeof($tholds)) {
		foreach ($tholds as $thold) {
			print "<option value='" . $thold['id'] . "'";

			if (get_request_var('threshold_id') == $thold['id']) {
				print ' selected';
			} print '>' . html_escape($thold['name_cache']) . '</option>';
		}
	}
	?>
						</select>
	...
	if (cacti_sizeof($thold_log_states)) {
		foreach ($thold_log_states as $key => $value) {
			...
			if (get_request_var('status') == $key) {
				print ' selected';
			} print '>' . html_escape($value['display']) . '</option>';
		}
	}
	?>
						</select>
