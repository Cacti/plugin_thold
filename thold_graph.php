...
include_once($config['base_path'] . '/plugins/thold/general_header.php');
...
function display_thold_logs () {
	...
	$total_rows_sql = "SELECT plugin_thold_log.*, thold_data.name FROM thold_data, plugin_thold_log
	...
	if (count($result)) {
		foreach ($result as $row) {
			...
			if ($row['status'] != 0) {
				$alertstat='yes';
				$bgcolor=($row['status'] > 1 ? 'red' : 'orange');
			} else {
				$alertstat='no';
				$bgcolor='green';
			};
			...
			print "<td width='50'>" . ($row['thold_type'] == 0 ? $row['thold_hi'] : ($row['thold_type'] == 2 ? $row['time_hi'] : '')) . "</td>";
			print "<td width='50'>" . ($row['thold_type'] == 0 ? $row['thold_low'] : ($row['thold_type'] == 2 ? $row['time_low'] : '')) . "</td>";
			print "<td width='80'>" . $row['current'] . "</td>";
			form_end_row();
		}
	} else {
		form_alternate_row_color($colors['alternate'],$colors['light'],0);
