...
function thold_check_treshold ($rra_id, $data_id, $name, $currentval, $cdef) {
	global $config;
...
//	$rrdsql = db_fetch_row("SELECT id FROM data_template_rrd WHERE local_data_id=$rra_id ORDER BY id ASC LIMIT 1");
//	$grapharr = db_fetch_assoc("SELECT DISTINCT local_graph_id FROM graph_templates_item WHERE task_item_id=" . $rrdsql["id"]);
	...
	$msg = "<html><body><a href=$httpurl/graph.php?local_graph_id=$graph_id&rra_id=1>$httpurl/graph.php?local_graph_id=$graph_id&rra_id=1</a><br>";
...
function save_thold() {
	...
	global $hostid;
        ...
        $_POST["bl_enabled"] = isset($_POST["bl_enabled"]) ? $_POST["bl_enabled"] : "off";
        $_POST["thold_enabled"] = isset($_POST["thold_enabled"]) ? $_POST["thold_enabled"] : "off";
