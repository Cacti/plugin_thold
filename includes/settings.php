...
function thold_config_settings () {
	...
	$settings['alerts'] = array(
		...
		'thold_sms_header' => array(
			'friendly_name' => __('SMS Sending Options', 'thold'),
			'method' => 'spacer',
		),
		'thold_enable_sms' => array(
			'friendly_name' => __('Enable SMS sending', 'thold'),
			'description' => __('If checked, this will cause Cacti to send SMS to numbers in Notification Lists.', 'thold'),
			'method' => 'checkbox',
			'default' => 'off',
		),
		'alert_phone' => array(
			'friendly_name' => __('Dead Host Notifications phone numbers for SMS', 'thold'),
			'description' => __('This is the comma separated list of phone numbers that the Dead Host Notifications will be sent to if the Global Notification List is selected.', 'thold'),
			'method' => 'textbox',
			'size' => 80,
			'max_length' => 255,
		),
